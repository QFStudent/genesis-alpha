...
from ga.sim.solvers.miso_tod_vc import MISOTODVC
...
class TwoSysMISOVC:
    ...
    def predict(self, solver_inst: MISOTODVC, test: pd.DataFrame):
        x = test.iloc[:, 1:]
        yhat = solver_inst._predict(x, self.zhat01.reshape(-1, 1), self.zhat02.reshape(-1, 1))
        return yhat

    def run_epoch(self, train: pd.DataFrame, test: pd.DataFrame) -> np.ndarray:
        solver_inst = self.fit_train(train)
        yhat = self.predict(solver_inst, test)
        return yhat
