...
from ga.sim.solvers import miso_tod_bayesian
...
class BAYESIAN_TOD_MISO:
    ...
    def __init__(self, numPoles1: int, numPoles2: int, penalty: float, a: float, b: float,
                 numInputs: int, tod_idx: List[str]):
    ...
    def fit_train(self, train: pd.DataFrame) -> TIBSystem:
        ...
        solver_inst = miso_tod_bayesian.MISOTODBAYESIAN(
            p1=self.numPoles1, p2=self.numPoles2, q=1, model="inverse", phi=1, tod_index=self.tod_idx
        )
    ...
    def predict(self, solver, raw_test: pd.DataFrame):
        x = raw_test.iloc[:, 1:]
        yhat = solver._predict(x, self.zhat01.reshape(-1, 1), self.zhat02.reshape(-1, 1))
        return yhat
